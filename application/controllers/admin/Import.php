...
class Import extends CI_Controller {
    ...
    public function readExcel($filename)
    {
        ...
        for ($row = 3; $row <= $highestRow;$row++)
        {
            ...
            // var_dump($this->Data_model->count_row('candidate',"idcard = '".$data['idcard']."'"));exit;
            if(($this->Data_model->count_row('candidate',array('email'=>$data['email'])) > 0 && ($data['email'] != '')) || ($this->Data_model->count_row('candidate',"idcard = '".$data['idcard']."'") > 0 && ($data['idcard'] != '')) || ($this->Data_model->count_row('candidate',"telephone LIKE '%".$data['telephone']."%'") > 0) ){
                array_push($arr_candidate, $candidateid);
                continue;
            }
            else{
